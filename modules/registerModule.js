...
exports.register = async (req, res, next) => {
    ...
    if(existUsername) return res.status(200).send({msg : "Username already exists.", status : "error"});

    var existNumber = await User.findOne({"number": req.body.number}).exec();
    if(existNumber) return res.status(200).send({msg : "Number already exists.", status : "error"});
